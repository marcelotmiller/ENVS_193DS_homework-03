...
```{r warning=FALSE}
flextable(caffeine_summary) |> 
  set_header_labels(
    'HW/Exam Due' = "Homework or Exam Due?",
    mean_caffeine = "Average Caffeine Intake (mg)") |> colformat_num(
      j = "mean_caffeine",
  ...
  bold(part = "header") |> 
  set_caption("Table: Average caffeine consumption depending on whether homework was due or an exam took place.")
```
...
affiliation, state population, population percentage of high school and college graduates, area, and area\
covered by water) were linked to recent renewable energy production in the United States. The statistical test used to test this was a multivariate linear regression analysis, where the primary predictor variable was the percentage of registered Democrats that reside in the state, and the primary response variable being the percentage of combined renewable energy in the state. As I mentioned in the homework two, "... relation between variables was found to be significant if the p-value was\
below the set value of 0.05, along with checking if there was a positive or negative relationship using the\
regression’s coefficient sign values."
